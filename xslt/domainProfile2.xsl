...
>
	...
	<xsl:template match="/">
		...
		<columns>
		<xsl:for-each select="//cl:clause[parent::chapter][number(substring-after(@id,'_'))&gt;=$startClause and number(substring-after(@id,'_'))&lt;=$endClause]//w[child::*[1][starts-with(name(),'VB') or self::NON or starts-with(name(),'AD')]][ancestor::cl:clause[parent::chapter]/*[not(local-name()='conj')][1]]">
				<xsl:variable name="dom">
					<xsl:choose>
						<xsl:when test="sem/*[@primarydomain]">
							<xsl:value-of select="sem/*[@primarydomain]/@majorNum"/>
						</xsl:when>
						<xsl:otherwise>
				...
				<xsl:if test="not(preceding::w[ancestor::cl:clause[parent::chapter]/*[not(local-name()='conj')][1]][child::*[1][starts-with(name(),'VB') or self::NON or starts-with(name(),'AD')]]/sem/*[@primarydomain]/@majorNum = $dom or preceding::w[ancestor::cl:clause[parent::chapter]/*[not(local-name()='conj')][1]][child::*[1][starts-with(name(),'VB') or self::NON or starts-with(name(),'AD')]]/sem/*[1]/@majorNum = $dom)">
					<col dom="{$dom}"/>
				</xsl:if>
		...
		<semantic_chains>
			<xsl:for-each select="//cl:clause[parent::chapter][number(substring-after(@id,'_'))&gt;=$startClause and number(substring-after(@id,'_'))&lt;=$endClause]">
				...
				<unit id="{substring-after($id,'.')}">
					...
					<xsl:for-each select="*[not(local-name()='conj')][1]//w[child::*[1][starts-with(name(),'VB') or self::NON or starts-with(name(),'AD')]]">
						<xsl:variable name="dom">
							<xsl:choose>
								<xsl:when test="sem/*[@primarydomain]">
									<xsl:value-of select="sem/*[@primarydomain]/@majorNum"/>
						...
						<xsl:if test="not(preceding::w[ancestor::cl:clause[@id=$id]/*[not(local-name()='conj')][1]][child::*[1][starts-with(name(),'VB') or self::NON or starts-with(name(),'AD')]]/sem/*[@primarydomain]/@majorNum = $dom or preceding::w[ancestor::cl:clause[@id=$id]][child::*[1][starts-with(name(),'VB') or self::NON or starts-with(name(),'AD')]]/sem/*[1]/@majorNum = $dom)">
							
								<dom num="{$dom}"><xsl:value-of select="count(ancestor::cl:clause[@id=$id]//w[child::*[1][starts-with(name(),'VB') or self::NON or starts-with(name(),'AD')]][sem/*[@primarydomain]/@majorNum = $dom]) + count(ancestor::cl:clause[@id=$id]//w[child::*[1][starts-with(name(),'VB') or self::NON or starts-with(name(),'AD')]][sem[not(dom/@primarydomain)]/*[1]/@majorNum = $dom])"/></dom>
