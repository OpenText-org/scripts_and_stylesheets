...
<test>
{
let $book := substring-after(request:request-uri(),'getText/')
let $xpath := replace(replace(request:request-parameter("xpath","//chapter[@num='1']"),"&lt;", "<"),"&gt;",">")

return
	 
	  util:eval(concat("document('/db/opentext/", $book, "/base/", $book, ".xml')", $xpath))
