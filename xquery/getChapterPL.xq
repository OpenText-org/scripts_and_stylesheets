...
declare function f:processChild($c) 
{
	...
	return
		...
		then
			...
			if ($nd='w')
			then
			(:		let $w := util:eval(concat("collection('/db/papyrus/", $book, "')"))//w[@id&=$c/@xlink:href]  :)
				let $w := collection('/db/papyrus')//w/@id[. &= $c/@xlink:href ]/parent::w
					return
...
let $doc :=  
		util:eval(concat("document('/db/papyrus/", $book, "/clause/", $book, "-cl.xml')"))
return
	<data>
	{
		if (exists($doc//chapter))
		then
				<chapter col="/papyrus/{$book}/clause" filename="{concat($book,'-cl')}">
		{
			$doc//chapter/@*
		}
		{
		...
		let $bk := util:eval(concat("document('/db/papyrus/", $book, "/base/", $book, ".xml')//text"))
	return
		<chapter  book="{$bk/@id}"  col="/papyrus/{$book}/clause" filename="{concat($book,'-cl')}">
...
<words>
	{
		let $wds := util:eval(concat("document('/db/papyrus/", $book, "/base/", $book, ".xml')//text//w"))
		for $w in $wds 
		return
